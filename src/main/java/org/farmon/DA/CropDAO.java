...
import org.farmon.JPA.CropJpaController;
...
//    
    ...
    public List<Crop> getActiveCropList() {
        EntityManager em = getEntityManager();
        TypedQuery<Crop> query = em.createNamedQuery("Crop.listActiveCrops", Crop.class);            
        List<Crop> listofcrop = query.getResultList();
        return listofcrop;
    }
...
//    }
    ...
    public Crop getCropForId(int id) {
        EntityManager em = getEntityManager();
        TypedQuery<Crop> query = em.createNamedQuery("Crop.cropForId", Crop.class); 

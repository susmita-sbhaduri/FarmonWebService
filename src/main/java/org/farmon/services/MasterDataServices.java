...
//        List<ResAcquireDTO> recordList = new ArrayList<>();
        ResAcquireDTO record = new ResAcquireDTO();
        try {
            Resourceaquire result = acqresdao.resAcqPerResid(Integer.parseInt(resid));

            record.setAcquireId(result.getAquireid().toString());
            record.setResoureId(String.valueOf(result.getResourceid()));
            record.setAmount(String.format("%.2f", result.getAmount().floatValue()));
            ...
            System.out.println(exception + " has occurred in getResAcqPerResid.");
            return null;
        }
    }
...
//                    record.setExpenseRefId("Paid Labor");
//                }
//                
//                if(reclist.get(i).getExpensetype().equals("SALARY")||
//                   reclist.get(i).getExpensetype().equals("LOAN")||
//                   reclist.get(i).getExpensetype().equals("BONUS")||
//                   reclist.get(i).getExpensetype().equals("LEAVE")){
//                   record.setExpenseType(reclist.get(i).getExpensetype()); 
//                   record.setExpenseRefId(getEmpNameForId(reclist.get(i).getExpenserefid().toString()).getName());
//                }               
//               
//                record.setExpenditure(String.format("%.2f", reclist.get(i).getExpediture()));
//                totalcost = totalcost + reclist.get(i).getExpediture().floatValue();
//                mysqlDate = reclist.get(i).getDate();
//                if (mysqlDate != null) {
//                    record.setDate(formatter.format(mysqlDate));
//                } else {
...
//                }
//                recordList.add(record);
//                record = new ExpenseDTO(); 
//                if (i == reclist.size() - 1) {
//                    record.setExpenseType("Total");
//                    record.setExpenseRefId("");
//                    record.setExpenditure(String.format("%.2f", totalcost));
//                    record.setDate("");
//                    recordList.add(record);
//                }
//            }
...
//    #################################Report batch#####################################################    
    ...
    public InventoryDTO getLastInvForSales(String cropid, String prodid, String harvestid, 
            Date salesdate) {
        InventoryDAO invdao = new InventoryDAO(utx, emf);
        ...
        SimpleDateFormat formatter = new SimpleDateFormat(pattern);
        try {
            Inventory invrecord = invdao.getLastInvForSales(Integer.parseInt(cropid),
                    Integer.parseInt(prodid), Integer.parseInt(harvestid), salesdate);
            record.setInventoryId(invrecord.getId().toString());
            record.setCropId(cropid);            
            record.setProductId(prodid);            
            record.setHarvestId(harvestid);            
            ...
            System.out.println("Inventory for this crop+product+harvest+salesdate");
            return null;
        } catch (Exception exception) {
            System.out.println(exception + " has occurred in getLastInvForSales().");
            return null;
        }
    }
